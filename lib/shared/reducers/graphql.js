...
// Generates an id for data with _id
const idsCounter = 0;
function generateID () {
  return `data${idsCounter}`;
}
    ...
    forEach(action.data, (data, graphAction) => {
      ...
      if (data.constructor === Array) { // multiple entries query or mutation
        ...
        if (action.params && action.params.remove) { // TODO: Is this the best way?
          ...
          Object.assign(changes, {
            [ID]: typeof data === 'object' ?
              Object.assign({}, state[ID] || {}, changes[ID] || {}, data) :
              data
          });
        }
        connectorData = ID;
            ...
            if (queries) {
              forEach(queries, (mutations, queryName) => {
                if (mutations[graphAction]) {
                  ...
                  const arrayConnectorData =
                    connectorData.constructor === Array ?
                    connectorData :
                    [connectorData];
