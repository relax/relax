...
describe('Media Reducer', () => {
  ...
  it('should handle ADD_MEDIA_FILES_TO_UPLOAD', () => {
    const action = {type: actionTypes.addFilesToUpload,
                   files: [{id: 1, name: 'File One'}, {id: 2, name: 'File Two'}]
                  };
    ...
    expect(mediaReducer(defaultState, action)).to.have.property('uploads')
                                              .that.is.an('array')
  ...
  it('should handle CHANGE_MEDIA_DISPLAY', () => {
    ...
    expect(mediaReducer(defaultState, action)).to.be.an('object');
    expect(mediaReducer(defaultState, action)).to.have.property('display').to.equal(action.display);
  });
  ...
  it('should handle UPLOADING_MEDIA', () => {
    ...
    expect(mediaReducer(defaultState, action)).to.be.an('object');
    expect(mediaReducer(localState, action)).to.have.property('uploads')
                                              .that.is.an('array')
                                              .with.deep.property('[0]')
