...
@connect(
  (state, props) => ({
    ...
    showMarks: props.showMarks !== false &&
              state.pageBuilder.editing &&
              !state.pageBuilder.linkingData &&
              (props.dropInfo.id === 'body' || (state.pageBuilder.selectedParent === props.dropInfo.id && state.pageBuilder.selectedElement && (!state.pageBuilder.selectedElement.children || state.pageBuilder.selectedElement.children.constructor !== Array)) || (state.pageBuilder.selectedId === props.dropInfo.id)),
