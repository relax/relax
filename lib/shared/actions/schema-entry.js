...
export function publishSchemaEntry (schemaId, id) {
  return mutation({
    fragments: {
      updateSchemaEntryState: {
        _id: 1,
        state: 1
      }
    },
...
export function unpublishSchemaEntry (schemaId, id) {
  return mutation({
    fragments: {
      updateSchemaEntryState: {
        _id: 1,
  ...
  return (dispatch, getState) => {
    ...
    return mutation({
      ...
      if (redirect) {
        ...
        dispatch(push(`/admin/schemas/${schemaType}/${schemaId}/${result.addSchemaEntry._id}`));
      } else {
        dispatch({
