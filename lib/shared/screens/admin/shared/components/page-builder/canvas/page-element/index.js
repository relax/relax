...
class PageElement extends Component {
  ...
  render () {
    ...
    if (element) {
      ...
      if (this.processedElement.displayElement) {
        const {ElementClass, props, hasLink} = this.processedElement;
  ...
  renderChildren (options = {}) {
    ...
    // options overlaps
    if (options.links) {
      this.processedElement.childrenLinks = options.links;
    }
    if (options.linksData) {
      this.processedElement.childrenLinksData = options.linksData;
    }
  ...
  renderChild (childId, position) {
    const {editing, building, updateStylesMap, draggingSelf, parentDragging} = this.props;
    const {childrenLinks, childrenLinksData, childrenContext} = this.processedElement;
    const {childrenEditable} = this.adminElement;
...
const mapStateToProps = (state, props) => {
  ...
  const dragInfo = state.dnd.dragInfo;
