...
app.use(async (req, res, next) => {
  ...
  const metaSettings = {
    ...
    'copyright':'copyright', 'robots':'robots', 'dctitle': 'DC.title'
  };

  const settingsArr = await SettingModel
    .find({
      _id: {$in: ['title', 'favicon'].concat(_keys(metaSettings))}
    })
    .exec();
  ...
  _forEach(metaSettings, ( (value, key) => {
    res.locals.header.push(
      {
        tag: 'meta',
        props:{
          name: value,
          content: settings[key] && safeHtmlString(settings[key]) || ''
        }
      }
    );
  }));
