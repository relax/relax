...
async function updatePage (id, changes, projection, userId) {
...
export const updateTitle = {
  ...
  async resolve (root, params, options) {
    ...
    return await updatePage(params.id, {title: params.title}, projection, root.user._id);
  }
};
...
export const updateSlug = {
  ...
  async resolve (root, params, options) {
    authorize(root);
    const projection = getProjection(options.fieldASTs[0]);
    return await updatePage(params.id, {slug: params.slug}, projection, root.user._id);
  }
};
