...
export default [
  ...
  // Settings
  {
    ...
    callback: (stores, render, req, next) => {
      stores.settings
        .findByIds(generalSettingsIds.concat(mailSettingsIds))
        .then((settings) => {
          render({
    ...
    callback: (stores, render, req, next) => {
      ...
      Q()
        ...
        .then((schemaEntriesStore) => Q.all([
          ...
          stores.schemas.findBySlug(req.params.slug),
          schemaEntriesStore.count()
        ]))
        .spread((schemaEntries, schema, count) => {
          render({
