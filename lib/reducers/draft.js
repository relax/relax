...
function doAction (data, action) {
  ...
  } else if (action.type === 'elementAddSchemaLink') {
    const element = data[action.elementId];
    const newLink = {
  ...
  } else if (action.type === 'elementRemoveSchemaLink') {
    const element = data[action.elementId];
    const currentSchemaLinks = element.props.schemaLinks;
    const newProperties = element.props.schemaLinks[action.propertyId].slice(0);
