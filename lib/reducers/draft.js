...
export default function draftReducer (state = defaultState, action = {}) {
  switch (action.type) {
    ...
    case actionTypes.makeElementSymbol:
      const elementToSymbol = state.data.data[action.params.elementId];
      const elementToSymbolParent = state.data.data[elementToSymbol.parent];
      const elementToSymbolPosition = elementToSymbolParent.children.indexOf(action.params.elementId);

      const makeElementSymbolActionResult = doAction(state.data.data, [
        {
          type: 'remove',
      ...
      return Object.assign({}, state, {
        ...
        data: Object.assign({}, state.data, {
          ...
          actions: [...state.data.actions, makeElementSymbolActionResult.revertAction]
        })
      });
